<template>
  ...
  >
    ...
    <Buttons ref="buttons" @toggle-theme-mode="toggleThemeMode" @stop="changePlay" />


    <BodyBgImg v-if="$themeConfig.bodyBgImg" />

    <!-- 背景动画 -->
    <canvas id="canvas" v-show="isPlay"></canvas>
  </div>
</template>
...
// 鼠标点击动效
// import mouseClick from "../mouseClick/love";
// 全局背景
import background1 from "../mouseClick/fireworks";
import background2 from "../mouseClick/background";
...
export default {
  ...
  mounted() {
    ...
    // mouseClick();
    // 修改鼠标样式
    // document.body.style.cursor = "url(resource/pic/icons/magnifier3.cur) 12 12,pointer";
  },
  watch: {
    isSidebarOpen() {
      if (this.isSidebarOpen) {
  ...
  methods: {
    ...
    // 当音乐暂停时，烟花暂停；当音乐播放时，音乐播放
    changePlay(data){
      this.isPlay = data;
      console.log("当前：" + this.isPlay);
    }
  }
};
</script>

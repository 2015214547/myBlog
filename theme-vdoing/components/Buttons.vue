<template>
  <div class="buttons">
    ...
    <div
      title="音乐播放"
      class="blur go-to-music xuanzhuan"
      :class="{ 'pause' : !isPlayMusic }"
      @click="handleMusic"
    >
      <!-- 音频播放 -->
      <audio
        :src="musicList[playIndex]"
        controls="controls"
        autoplay="autoplay"
        preload="auto"
        id="audio"
        style="width:0%;height:0px;outline:none;opacity:0;"
...
export default {
  ...
  mounted() {
    ...
    // 随机定义播放的歌曲
    this.randomNum(0, this.musicList.length - 1);
    // 初始化完成后播放音乐
    this.handleMusic();
  },
  computed: {
  ...
  methods: {
    ...
    playEndedHandler() {
      this.randomNum(0, this.musicList.length - 1);
      let audio = document.querySelector("#audio");
      audio.src = this.musicList[this.playIndex];
    },
    // 生成minNum~maxNum之间的随机整数
    randomNum(minNum, maxNum) {
      switch (arguments.length) {
        case 1:
          this.playIndex = parseInt(Math.random() * minNum + 1, 10);
          break;
        case 2:
          this.playIndex = parseInt(
